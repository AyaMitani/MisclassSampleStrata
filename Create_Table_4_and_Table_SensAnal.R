...
designbased_tr <- function(cutoff){
  findata$svywt_tr <- ifelse(findata$svywt > cutoff, cutoff, findata$svywt)
  design_tr <- svydesign(id = ~patient.id, strata = ~ e.race.eth.5 + e.agelt35 + e.female + e.rural + e.edugrp, weights = ~svywt_tr, data = findata)
...
### design agnostic
designagnostic <- glm(s.trust ~ as.factor(s.race.eth.5) + s.poverty + s.agelt35 + s.female + s.rural + s.edu_L + s.edu_M, data = findata, family = binomial("logit"))
sumdesignagnostic <- summary(designagnostic)

### model-based (adjusting for stratification variables)
modelbased <- glm(s.trust ~ as.factor(s.race.eth.5) + s.poverty + s.agelt35 + s.female + s.rural + s.edu_L + s.edu_M + as.factor(e.race.eth.5) + e.agelt35 + e.female + e.rural + as.factor(e.edugrp), data = findata, family = binomial("logit"))
...
design <- svydesign(id = ~patient.id, strata = ~ e.race.eth.5 + e.agelt35 + e.female + e.rural + e.edugrp, weights = ~svywt, data = findata)
designbased <- svyglm(s.trust ~ as.factor(s.race.eth.5) + s.poverty + s.agelt35 + s.female + s.rural + s.edu_L + s.edu_M, design = design, family = quasibinomial("logit") )
sumdesignbased <- summary(designbased)

### design-based (weighted analysis) with weights truncated at 90th percentile
cutoff <- quantile(findata$svywt, 0.90)
findata$svywt_tr <- ifelse(findata$svywt > cutoff, cutoff, findata$svywt)
design_tr <- svydesign(id = ~patient.id, strata = ~ e.race.eth.5 + e.agelt35 + e.female + e.rural + e.edugrp, weights = ~svywt_tr, data = findata)
designbased_tr <- svyglm(s.trust ~ as.factor(s.race.eth.5) + s.poverty + s.agelt35 + s.female + s.rural + s.edu_L + s.edu_M, design = design_tr, family = quasibinomial("logit") )
sumdesignbased_tr <- summary(designbased_tr)
...
sumdesignbased_tr100 <- designbased_tr(quantile(findata$svywt, 1))
sumdesignbased_tr99 <- designbased_tr(quantile(findata$svywt, 0.99))
sumdesignbased_tr95 <- designbased_tr(quantile(findata$svywt, 0.95))
